<div class="mt-8 space-y-4 px-8">
    <form class="flex flex-wrap justify-center" action="" method="GET" wire:submit="find">
        <div class="flex w-full basis-full gap-1 align-middle sm:gap-2 xl:max-w-4xl">
            <input class="border flex basis-5/6 border-solid border-gray-300 p-2 text-2xl xs:text-base sm:text-3xl"
                name="serial_number" type="text" placeholder="{{ __('Search product') }}" wire:model="serial_number" />
        ...
        @if ($product !== null && $product->exists())
            <div
                ...
                <div class="basis-1/2 sm:basis-1/2 md:basis-2/3">
                    ...
                    <p class="basis-full text-gray-500">{{ $product->serializeDate($product->warrantee_date) }}</p>
                </div>
                @if ($owns < 1)
                ...
                @else
                    <a class="btn btn-primary m-2" href="{{ route('products.edit', ['product' => $product->id]) }}"
                        wire:navigate>
                        <button
