<div class="mt-8 space-y-4 px-8">
    <form action="" method="GET" wire:submit="find" class="flex flex-wrap justify-center">
        <div class="flex w-full basis-full gap-1 align-middle sm:gap-2 xl:max-w-4xl">
            <input class="border flex basis-5/6 border-solid border-gray-300 p-2 text-2xl xs:text-5xl sm:text-3xl"
            ...
            <div
                ...
                @if ($owns)
                    <a wire:navigate href="{{ route('products.add', ['product' => $product->id]) }}">
                        <button
                            class="focus:shadow-outline inline max-h-10 items-center justify-center rounded bg-primary px-4 font-bold text-white hover:bg-blue-700 focus:outline-none xs:text-xs sm:max-h-14 sm:basis-1/2 sm:text-3xl md:basis-1/3 md:text-2xl">
                            {{ __('add to me') }}
                        </button>
                    </a>
                @else
